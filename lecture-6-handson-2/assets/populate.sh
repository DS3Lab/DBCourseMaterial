...
docker exec --workdir /var/lib/postgresql postgres-sever wget -O discogs.zip https://cloud.inf.ethz.ch/s/PWRFqmNczEkYS7M/download
...
docker exec postgres-server sh -c "psql -U postgres discogs < /var/lib/postgresql/discogs/inserts.sql"
